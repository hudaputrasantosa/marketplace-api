...
const setorSaldo = async(req, res) => {
    try {
        ...
        if (checkDompet && saldo >= 10000) {
            ...
            res.status(404).json({
                status: "Notfound",
                message: "Anda belum mempunyai dompet/setor saldo minimal 10000",
            });
        }
    } catch (error) {
...
const tarikSaldo = async(req, res) => {
    try {
        ...
        if (checkDompet) {
            const checkSaldo = checkDompet.saldo >= saldo && saldo >= 10000;
            if (!errors.isEmpty()) {
                return res.status(400).json({
                    status: "Bad Request",
                    errors: errors.array(),
            ...
            if (checkSaldo) {
                ...
                res.status(409).json({
                    status: "Conflict",
                    message: "Jumlah tarik saldo tidak mencukupi saldo anda saat ini atau minimal tarik saldo 10000",
                });
            }
        } else {
            res.status(404).json({
